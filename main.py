...
if response.text == version_actuelle:
    ...
    def main():
        root = tk.Tk()
        root.title("RPC Living Room v" + version_actuelle)
    ...
    label = tk.Label(root, text="Une nouvelle version est disponible !\nVersion : {}".format(response.text), font=("Helvetica", 20), bg="#2C2F33", fg="white")
    label.pack(pady=10)
